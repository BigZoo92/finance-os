...
- API endpoints `compose.update`, `compose.one`, `environment.update`, `compose.deploy`
...
2. mise a jour du `docker-compose.prod.yml` dans Dokploy via `compose.update`
3. mise a jour de `APP_IMAGE_TAG`, `APP_VERSION`, `APP_COMMIT_SHA` dans l'env Dokploy
