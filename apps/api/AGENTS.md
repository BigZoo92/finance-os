...
- Keep the advisor stack (`/dashboard/advisor`, `/dashboard/advisor/*`, `/dashboard/manual-assets`, persisted runs/artifacts/cost ledger, grounded chat, challenger flow, admin-triggered `POST /dashboard/advisor/manual-refresh-and-run`, optional worker-triggered `POST /dashboard/advisor/run-daily`) aligned with `docs/AI-ARCHITECTURE.md` and `docs/AI-SETUP.md`: demo must stay fully deterministic and write-free, admin/internal-token mutations must remain guarded, and GET routes must never trigger live provider work.
