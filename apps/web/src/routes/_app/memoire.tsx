...
function MemoirePage() {
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        eyebrow="AI Advisor memory"
        icon="[#]"
        title="Memoire & connaissances"
        description="Graphe temporel interne pour enrichir, expliquer et challenger les recommandations sans remplacer le moteur deterministe."
      />

      <section className="grid gap-4 md:grid-cols-4">
        <Metric
          label="Entites"
          value={statsQuery.data?.entityCount ?? 0}
          loading={statsQuery.isPending}
        />
        <Metric
          label="Relations"
          value={statsQuery.data?.relationCount ?? 0}
          loading={statsQuery.isPending}
        />
        <Metric
          label="Contradictions"
          value={statsQuery.data?.contradictionCount ?? 0}
          loading={statsQuery.isPending}
        />
        <Metric
          label="Latence query"
          value={`${Math.round(statsQuery.data?.queryLatencyMs ?? 0)} ms`}
          loading={statsQuery.isPending}
        />
      </section>
      ...
      <Panel
        ...
        description="BM25, similarite semantique locale, traversal relationnel, temporalite, confiance et provenance."
      ...
      <section className="grid gap-6 lg:grid-cols-[minmax(0,0.92fr)_minmax(0,1.08fr)]">
        <Panel title="Concepts trouves" tone="violet" icon={<span aria-hidden="true">::</span>}>
          ...
          {selectedHit ? (
            <div className="space-y-5">
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <Badge variant="secondary">
                    {percent(selectedHit.entity.confidence)} confiance
                  </Badge>
                  <Badge variant="secondary">{selectedHit.entity.scope}</Badge>
                </div>
                <h2 className="mt-3 text-xl font-semibold tracking-tight">
                  {selectedHit.entity.label}
                </h2>
                <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
                  {selectedHit.entity.description}
                </p>
              </div>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <Temporal label="Observe" value={selectedHit.entity.observedAt ?? null} />
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  {selectedHit.relations.length ? (
                    selectedHit.relations.slice(0, 10).map(relation => (
                      <Badge key={relation.id} variant="outline" className="max-w-full truncate">
                        {relation.type}
                      </Badge>
                    ))
                  ) : (
                    <span className="text-sm text-muted-foreground">
                      Aucune relation proche dans cette reponse.
                    </span>
                  )}
                </div>
              </div>
            ...
            <p className="py-8 text-sm text-muted-foreground">Selectionnez un concept.</p>
          )}
        </Panel>
      </section>
      ...
      <section className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
        <Panel
          title="Context bundle AI Advisor"
          tone="positive"
        ...
        <Panel title="Schema & rebuild" tone="plain" icon={<span aria-hidden="true">{'{}'}</span>}>
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <p className="text-sm text-muted-foreground">
                Vector:{' '}
                <span className="text-foreground">{statsQuery.data?.vectorBackend ?? '-'}</span>
              </p>
              <p className="text-sm text-muted-foreground">
                Dernier rebuild:{' '}
                <span className="text-foreground">
                  {formatDateTime(statsQuery.data?.lastSuccessfulRebuildAt ?? null)}
                </span>
              </p>
              <p className="text-sm text-muted-foreground">
                Schema:{' '}
                <span className="text-foreground">{schemaQuery.data?.schemaVersion ?? '-'}</span>
              </p>
            </div>
            ...
            {!isAdmin ? (
              <p className="text-xs text-muted-foreground">
                Mode demo: consultation deterministic only, aucune mutation.
              </p>
            ) : null}
    ...
    >
      ...
      <div className="mt-3 flex flex-wrap gap-1.5">
        {hit.entity.tags.slice(0, 4).map(tag => (
          <span
            key={tag}
            className="rounded bg-background px-2 py-1 text-[11px] text-muted-foreground"
          >
            {tag}
          </span>
        ))}
